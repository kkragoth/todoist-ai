Use the `@/` import alias for all cross-file imports (it maps to `src/` via tsconfig `paths` + Vite `resolve.alias`); never use relative `./` or `../` imports.
Model state as ADTs: use `enum` + discriminated unions with exhaustive `switch` and predicate/transition helpers in `src/lib/`; never compare raw string literals (e.g. `status === 'done'`) at call sites.
