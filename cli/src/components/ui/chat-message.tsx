...
const wrapPlainChildren = (node: ReactNode, selectable?: boolean): ReactNode =>
  typeof node === "string" || typeof node === "number" ? (
    <text selectable={selectable}>{node}</text>
  ) : (
    node
  );
  ...
  const renderContent = () => {
    if (streaming) {
      return (
        <box>
          {children ? (
            wrapPlainChildren(children, selectable)
          ) : (
    ...
    if (collapsedProp) {
      return (
        <box>
          <text fg="#666" selectable={selectable}>{`${firstLine.slice(0, 60)}${firstLine.length > 60 || childrenText.includes("\n") ? "..." : ""}`}</text>
        </box>
      );
    }
